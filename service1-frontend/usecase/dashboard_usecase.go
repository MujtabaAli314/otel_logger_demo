...
func NewDashboardUsecase(data repository.DataClient, fraud repository.FraudClient, logger slog.Logger) DashboardUsecase {
...
func (u *dashboardUsecase) GetDashboard(ctx context.Context, params types.GetDashboardParams) (*types.DashboardResponse, error) {
	...
	u.logger.InfoContext(ctx, "Heeeey we reached the usecase!!!!!", "user_id", params.UserID)
	...
	if uErr != nil {
		if repository.IsNotFound(uErr) {
			...
			u.logger.ErrorContext(ctx, "usecase calling repo isnotfound and got an error", "user_id", params.UserID)
		...
		u.logger.ErrorContext(ctx, "usecase throwing an error"+uErr.Error())
		return nil, uErr
	}
	if tErr != nil {
		u.logger.ErrorContext(ctx, "usecase throwing an error"+tErr.Error())
		return nil, tErr
	}

	span.SetAttributes(attribute.Int("usecase.tx_count", len(txs)))
	u.logger.InfoContext(ctx, "fetched user and transactions", "user_id", params.UserID, "tx_count", len(txs))

	assessment, err := u.fraud.AssessFraud(ctx, params.UserID, txs)
	if err != nil {
		u.logger.ErrorContext(ctx, "usecase throwing an error"+err.Error())
		return nil, err
	}
	...
	u.logger.InfoContext(ctx, "fraud assessment complete", "risk_level", assessment.RiskLevel, "risk_score", assessment.RiskScore)
